<template>
  <div>
    <p class="lead">Now, the fun part begins: attempting to break Ed25519 in various ways. Our first approach is
      <a href="https://en.wikipedia.org/wiki/Malleability_(cryptography)">malleability</a>, the ability to create digital signatures without the knowledge of the necessary
      secret key.</p>
    ...
    <p>Contrary to a threatening definition, malleability doesn’t break a digital signature scheme. It allows to create
      ...
      under the message <code>M</code> and the public key <code>A</code>, produce another signature <code>(R′, s′) != (R, s)</code>
    ...
    <p>Although malleability is not a problem for signing, things can get ugly if a system
      ...
      before <a href="https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki">SegWit</a>). In this case, the same message signed by two different signatures will have
      different hashes and thus will be treated as two separate entities! This was a significant obstacle for
      implementing chained payments, e.g., for <a href="https://lightning.network/">Lightning Network</a>, in Bitcoin pre-SegWit.</p>

    <h3>Mutating signature scalar</h3>
    <p>Some Ed25519 implementations verify that signature scalar <code>s &lt; 2<sup>253</sup></code>, rather than comparing it
      to the subgroup order <code>ℓ</code>. The problem is, for most valid scalars <code>s</code>,
      <code>s + ℓ &lt; 2<sup>253</sup></code> as well; thus, <code>(R, s + ℓ)</code> is considered a valid signature.
      Since <code>2ℓ > 2<sup>253</sup></code>, using <code>s + 2ℓ</code>, <code>s + 3ℓ</code>, etc. as a scalar component
      of the signature will always fail.</p>
    ...
    <p class="mt-3">You can plug the message, public key and the new signature into <router-link target="_blank" :to="{ name: 'basics', hash: '#verification' }">verification form</router-link>
      or <a target="_blank" href="https://tweetnacl.js.org/#/sign">an external website</a> and check that the new signature is actually valid.</p>
    ...
    <p>It is impossible for an external party to mutate the <code>R</code> part of a signature, as it is included
      ...
      create as many signatures for the same content as he wishes. Indeed, it’s impossible to tell during signature verification if the point <code>R = [r]B</code>
    ...
    <data-row name="Message" :data="message" wrapper="$">Copied from the previous example together with the keypair.</data-row>
    <data-row name="Public key" :data="repr(keypair.publicKey().bytes())" wrapper='A = Pt("$")'></data-row>
    ...
    <p class="mt-3">If you search for a cryptographic primitive that provides
      a “unique” analogue of digital signatures, it does exist and is called <a href="https://en.wikipedia.org/wiki/Verifiable_random_function"><em>verifiable random function</em></a> (VRF);
      there is <a href="https://signal.org/docs/specifications/xeddsa/#vxeddsa">a VRF implementation based on Curve25519</a> as well.</p>
  </div>
</template>
<script>
